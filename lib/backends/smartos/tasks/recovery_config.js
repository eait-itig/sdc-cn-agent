...
function start() {
    ...
    vasync.pipeline({funcs: [
        function bailIfMissingPreconditions(_, cb) {
        ...
        function retrieveKbmapiAddresses(_, cb) {
        ...
        function onlyStagedConfigCanBeActivated(_, cb) {
            if (action === 'activate' &&
        ...
        function getSysinfoUpdated(_, cb) {
            execFile('/usr/bin/sysinfo', ['-f'], function (err, stdo, stde) {
                if (err) {
                    cb(Error(stde.toString()));
                    return;
         ...
         */
        function postUpdatesToKbmapi(_, cb) {
