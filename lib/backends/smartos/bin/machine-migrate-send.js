...
function SyncHandler(opts, event, socket) {
    ...
    // For docker shared volumes (where a volume is lofs mounted inside the
    // container).
    if (self.isDocker && Array.isArray(self.vm.filesystems)) {
        assert.string(self.vm.zfs_filesystem, 'vm.zfs_filesystem');
        assert.string(self.vm.zonepath, 'vm.zonepath');

        var volRegex = new RegExp('^' + self.vm.zonepath +
            '/volumes/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-' +
            '[a-f0-9]{4}-[a-f0-9]{12})$');

        self.vm.filesystems.some(function _someFs(f) {
            if (f.type === 'lofs' && f.source && f.source.match(volRegex)) {
                // Add the '.../volumes' dataset.
                var zfsFilesystem = self.vm.zfs_filesystem + '/volumes';
                self.datasets[zfsFilesystem] = {
                    zfsFilesystem: zfsFilesystem
                };
                self.log.info({zfs_filesystem: zfsFilesystem},
                    'added docker shared zfs volume to the dataset list');
                return true;
            }
            return false;
        });
    ...
    // For delegate datasets, there is a special zfs filesystem that holds user
    // created datasets, which must be sync'd separately.
    if (Array.isArray(self.vm.datasets) && self.vm.datasets.length) {
        assert.equal(self.vm.datasets.length, 1,
            'Should be only one delegate dataset');
        assert.arrayOfString(self.vm.datasets, 'self.vm.datasets');

        var delegateFs = self.vm.datasets[0];
        self.datasets[delegateFs] = {
            zfsFilesystem: delegateFs
        };
    }
}
...
function _syncHandlerConnectToReceiver(ctx, callback, retries) {
    assert.object(ctx, 'ctx');
    ...
    sock.on('error', function _onSocketError(err) {
        ...
        if (--retries > 0) {
            _syncHandlerConnectToReceiver(ctx, callback, retries);
        } else {
            sock.destroy();
    ...
    function onSockConnect() {
        ...
        /*
         * Disable the idle timeout now: we likely won't hear anything until
         * the send stream is finished.
         */
        sock.setTimeout(0);
...
function _syncHandlerGetSourceZfsSnapshotNames(ctx, callback) {
    ...
    child_process.execFile(cmd, args, gExecFileDefaults,
        ...
        lines = lines.filter(function (line) {
            return line.startsWith(ctx.zfsFilesystem + '@');
        });

        var allSourceSnapshotNames = lines.map(function _lineMap(line) {
            return line.split('@').splice(-1)[0].trim();
        });
        var oldestSnapshot = allSourceSnapshotNames[0];

        ctx.sourceSnapshotNames = allSourceSnapshotNames.filter(
                function _nameFilter(name) {
        ...
        if (self.isDocker && oldestSnapshot &&
                oldestSnapshot !== ctx.sourceSnapshotNames[0]) {
            ctx.sourceSnapshotNames.unshift(oldestSnapshot);
        }
...
function _syncHandlerGetTargetZfsSnapshotNames(ctx, callback) {
    ...
    this.runTargetCommand(ctx.receiverSocket, command,
        ...
        var allTargetSnapshotNames = event.names;

        if (!Array.isArray(allTargetSnapshotNames)) {
            log.warn('getTargetZfsSnapshotNames:: not an array!?');
            allTargetSnapshotNames = [];
        }

        // Note the we receive all snapshots (not just the migration snapshots),
        // so filter those down to the ones we are interested in.
        ctx.targetSnapshotNames = allTargetSnapshotNames.filter(
                function _nameFilter(name) {
            return name.startsWith(SNAPSHOT_NAME_PREFIX);
        });

        if (self.isDocker &&
                allTargetSnapshotNames[0] !== ctx.targetSnapshotNames[0]) {
            ctx.targetSnapshotNames.unshift(allTargetSnapshotNames[0]);
        }
...
function _syncHandlerDetermineSnapshotNames(ctx, callback) {
    ...
    if (ctx.isFirstSync && this.isDocker) {
        assert.equal(ctx.sourceSnapshotNames.length, 1,
            'expected exactly one source snapshot name');

        ctx.prevSnapshotName = '';
        ctx.snapshotName = ctx.sourceSnapshotNames[0];
        log.debug({
                prevSnapshotName: ctx.prevSnapshotName,
                snapshotName: ctx.snapshotName,
                zfsFilesystem: ctx.zfsFilesystem
            }, 'determineSnapshotNames:: no previous docker snapshots, but ' +
    ...
    if (!ctx.prevSnapshotName.startsWith(SNAPSHOT_NAME_PREFIX)) {
        assert.equal(this.isDocker, true, 'Only occurs for docker instances');
        // The prevSnapshotName fills in for the '1' snapshot, so start at '2'.
        ctx.snapshotName = SNAPSHOT_NAME_PREFIX + '2';
    } else {
        ctx.snapshotName = SNAPSHOT_NAME_PREFIX +
            (Number(ctx.prevSnapshotName.substr(SNAPSHOT_NAME_PREFIX.length))
            + 1);
    }
...
function _syncHandlerCleanupSourceSnapshots(ctx, callback) {
    ...
    function deleteOneSourceSnapshot(snapshotName, next) {
        if (!snapshotName.startsWith(SNAPSHOT_NAME_PREFIX)) {
            log.info('Snapshot name "%s" is not a migration snapshot',
                snapshotName);
            next();
            return;
        }
...
function _syncHandlerCleanupTargetSnapshots(ctx, callback) {
    ...
    if (!ctx.prevSnapshotName.startsWith(SNAPSHOT_NAME_PREFIX)) {
        log.info('Previous snapshot name "%s" is not a migration snapshot',
            ctx.prevSnapshotName);
        callback();
        return;
    }
